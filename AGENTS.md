...
## 核心功能
1. **全页刷新模式**：按设定间隔刷新当前标签页，等待页面加载完成后再开始下一轮倒计时
2. **XPath 点击模式**：按设定间隔对指定 XPath 元素执行模拟点击
3. **XPath 元素选取**：鼠标悬停高亮 + 点击选取，自动生成 XPath，选取后弹窗提示并自动启动刷新
4. **iframe 检测**：自动检测页面中的 iframe，支持选择在目标 iframe 内执行点击
5. **定时管理**：使用 setTimeout + chrome.alarms 双重机制，防止 Service Worker 被杀后定时器丢失
6. **间隔热切换**：运行中可随时修改间隔时间（预设或自定义），立即生效
7. **模式热切换**：运行中可在全页刷新与 XPath 点击模式间自由切换，无需反复启停
8. **刷新次数上限**：可设定最大刷新次数（默认无限制），达到上限自动停止
9. **白色冷色调主题**：白色背景 + 蓝色冷色调点缀

## 消息通信
- popup → background: START / STOP / GET_STATUS / UPDATE_INTERVAL / UPDATE_MODE / UPDATE_MAX_COUNT
- background → popup: REFRESH_DONE / REFRESH_ERROR / XPATH_PICKED（含元素信息）
- popup → content: START_PICKING / STOP_PICKING / DETECT_IFRAMES / EXECUTE_XPATH_CLICK
- content → background → popup: XPATH_PICKED（含 tagName、textContent）
